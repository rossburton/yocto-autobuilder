...
class GetDistroVersion(ShellCommand):
    ...
    def start(self):
        ...
        if 'poky' in self.distro:
            distroconf=builddir + "/build/meta-yocto/conf/distro/poky.conf|grep 'DISTRO_VERSION = \"'"
        elif 'oecore' in self.distro:
            distroconf=builddir + "/build/meta/conf/distro/include/default-distrovars.inc|grep 'DISTRO_VERSION '"
        else:
