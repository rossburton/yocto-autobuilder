...
class PublishArtifacts(ShellCommand):
    ...
    def start(self):
        ...
        self.basedir=os.path.join(builddir, "build", "build")
        command=""
        ...
        if str(os.environ.get('PUBLISH_BUILDS')) == "True":
            for artifact in self.artifacts:
                ...
                elif artifact == "sources":
                    command=command+"mkdir -p " + os.path.join(DEST, SOURCE_PUBLISH_DIR) + ";"
                    command=command+"cp -R --no-dereference --preserve=links " + \
                                    os.path.join(self.basedir, "tmp/deploy/sources/* ") + \
                                    os.path.join(DEST, SOURCE_PUBLISH_DIR) + ";"
